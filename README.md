...
Turbo Tour is configured to publish to RubyGems automatically when a GitHub release is published.
...
- verify that the release tag matches `TurboTour::VERSION`
- run the test suite
- build the gem
- publish the gem to RubyGems using trusted publishing

### One-time RubyGems Setup

Configure a trusted publisher for the `turbo_tour` gem on RubyGems.org:

- Owner: `dhairyagabha`
- Repository: `turbo_tour`
- Workflow filename: `release.yml`

No RubyGems API key secret is required when trusted publishing is configured.

Official setup guides:

- RubyGems trusted publishing: <https://guides.rubygems.org/trusted-publishing/>
- GitHub Actions publishing concepts: <https://docs.github.com/en/actions/concepts/use-cases/about-packaging-with-github-actions>

### Release Flow

1. Update `lib/turbo_tour/version.rb`.
2. Update `CHANGELOG.md` with the release notes for the new version.
3. Merge the release commit to the default branch.
4. Create a GitHub release with a tag that matches the gem version, such as `v0.1.0` or `0.1.0`.
5. Publish the release on GitHub.

Once the release is published, GitHub Actions will run the release workflow and push the gem to RubyGems automatically.
